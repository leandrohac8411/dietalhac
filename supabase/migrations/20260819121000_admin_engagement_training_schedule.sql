-- Atualiza a pontuação administrativa para considerar os dias de treino
-- configurados pelo usuário, preservando o ciclo contínuo A/B/C.
  ...
  scored AS (
    SELECT
      ...
      EXISTS (
        ...
        JOIN public.workout_plans wp ON wp.user_id = up.user_id AND wp.is_active
        WHERE up.user_id = pd.user_id
          AND EXTRACT(DOW FROM pd.activity_date)::INTEGER = ANY(
            COALESCE(up.training_weekdays, ARRAY[1,3,5])
          )
      ) AS workout_expected,
