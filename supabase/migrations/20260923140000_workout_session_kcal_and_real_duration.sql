-- 1) Guarda o gasto calórico estimado de cada treino, junto da duração real
--    (duration_min já existe, mas até aqui era sempre a duração PLANEJADA da
--    ficha, não o tempo que a pessoa realmente levou no modo ao vivo).
ALTER TABLE public.workout_sessions
  ADD COLUMN IF NOT EXISTS estimated_kcal INTEGER;

-- 2) complete_workout_cycle passa a aceitar o kcal estimado (calculado no
--    cliente a partir do tempo real decorrido) e gravá-lo junto da sessão,
--    tanto ao reaproveitar a sessão aberta pelo modo ao vivo quanto ao criar
--    uma nova.
DROP FUNCTION IF EXISTS public.complete_workout_cycle(UUID, INTEGER, UUID);

CREATE OR REPLACE FUNCTION public.complete_workout_cycle(
  p_workout_id UUID,
  p_duration_min INTEGER DEFAULT NULL,
  p_session_id UUID DEFAULT NULL,
  p_estimated_kcal INTEGER DEFAULT NULL
)
RETURNS JSONB
LANGUAGE plpgsql
SECURITY DEFINER
  ...
  ELSE
    ...
    IF p_session_id IS NOT NULL THEN
      ...
      SET finished_at = finished_time,
          ...
          estimated_kcal = COALESCE(p_estimated_kcal, estimated_kcal),
    ...
    ELSE
      INSERT INTO public.workout_sessions (
        ...
        started_at, finished_at, duration_min, estimated_kcal
      ) VALUES (
        uid, selected_workout.id, selected_plan.id, selected_workout.name, expected_position,
        finished_time - make_interval(mins => duration_value), finished_time, duration_value, p_estimated_kcal
...
REVOKE ALL ON FUNCTION public.complete_workout_cycle(UUID, INTEGER, UUID, INTEGER) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.complete_workout_cycle(UUID, INTEGER, UUID, INTEGER) TO authenticated;
